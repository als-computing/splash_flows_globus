...
def prune_spot832(file: str, if_older_than_days: int):
...
def prune_many_data832(project_dir_filter: str, older_than_days: int = 30, test: bool = False):
    ...
    nersc_project_dir_path = config.nersc.full_path(project_dir['name'])
    ...
    nersc_files_future =  get_files(config.tc, config.nersc, nersc_project_dir_path, [], older_than_days)
    ...
    for data832_file in data832_files:
        nersc_file_analog = config.nersc.root_path + data832_file.split(config.data832.root_path)[1]
