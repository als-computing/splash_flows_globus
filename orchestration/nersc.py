...
class NerscClient():
    ...
    def __init__(self, path_client_id = None, private_key = None, logger = None):

        self.path_client_id = default_path_client_id if path_client_id is None else path_client_id
        self.path_private_key = default_path_private_key if private_key is None else private_key
        self.logger = logging.basicConfig(level=logging.INFO) if logger is None else logger
        # if path_client_id is None: path_client_id = default_path_client_id
        # if private_key is None: private_key = default_path_private_key
        # if logger is None: self.logger = logging.basicConfig(level=logging.INFO)
    ...
    def request_task_status(self):
        r = self.session.get("https://api.nersc.gov/api/v1.2/tasks/{}".format(self.task_id))
        self.task = r.json()


    def request_job_status(self):
        r = self.session.get("https://api.nersc.gov/api/v1.2/compute/jobs/perlmutter/{}".format(self.jobid))
        self.job = r.json()
    ...
    def cdtools(self, cxiname, n_gpu, **kwargs):
        args_string = cdtool_args_string(cxiname, default_path_cdtools_nersc, cdtools_parms, **kwargs)
        job_script = get_job_script(n_gpu, args_string)
        self.logger.info(f"Job script: {job_script}")

        self.submit_job(job_script)
        self.task_wait()

    # def ptychocam(self, cxiname, n_gpu, **kwargs)
    ...
    def task_wait(self, max_wait_seconds=600, sleep_time = 2):
        ...
        while not done:
            ...
            if elapsed > max_wait_seconds:
                ...
                self.logger.info(f"Configured to wait {max_wait_seconds}, elapsed is {elapsed} ")
                self.logger.info(f"Job may complete in background.")

            if 'id' not in list(self.task.keys()):
                self.logger.info(f"Task {self.task_id} not found and may have been completed")
                done = True
