...
def recon_setup(
    ...
    corFunction='pc',  # center of rotation function to use - can be 'pc', 'vo', or 'nm',
                       # or use 'skip' to return tomo variable without having to do a calc.
    corLoadMinimalBakDrk=True,  # during cor detection, only load the first dark field and first flat field
                                # rather than all of them, to minimize file loading time for cor detection.
    ...
    projused=None,
    # should be slicing in projection dimension (start,end,step)
    # Be sure to add one to the end as stop in python means the last value is omitted
    sinoused=None,
    # should be sliceing in sinogram dimension (start,end,step).
    # If first value is negative, it takes the number of slices from the second value in the middle of the stack.
    correcttilt=0,  # tilt dataset
    tiltcenter_slice=None,  # tilt center (x direction)
    tiltcenter_det=None,  # tilt center (y direction)
    angle_offset=0,
    # this is the angle offset from our default (270) so that tomopy yields output
    # in the same orientation as previous software (Octopus)
    anglelist=None,
    # if not set, will assume evenly spaced angles which will be calculated by the angular range
    # and number of angles found in the file. if set to -1, will read individual angles from each image.
    # Alternatively, a list of angles can be passed.
    doBeamHardening=False,
    # turn on beam hardening correction, based on "Correction for beam hardening in computed tomography",
    # Gabor Herman, 1979 Phys. Med. Biol. 24 81
    BeamHardeningCoefficients=None,  # 6 values,
    # tomo = a0 + a1*tomo + a2*tomo^2 + a3*tomo^3 + a4*tomo^4 + a5*tomo^5
    projIgnoreList=None,
    # projections to be ignored in the reconstruction (for simplicity in the code, they will not be removed and
    # will be processed as all other projections but will be set to zero absorption right before reconstruction.
    ...
    *args, **kwargs
    ...
    # outputPath = inputPath + 'rec' + os.path.splitext(filename)[0] +
    # '/' if outputPath is None else outputPath + 'rec' + os.path.splitext(filename)[0] + '/'
    outputPath = (os.path.join(inputPath, 'rec' + outputFilename)
                  if outputPath is None
                  else os.path.join(outputPath, 'rec' + outputFilename))
    ...
    if (filetype == 'als') or (filetype == 'als1131'):
        ...
        if 'num_dark_fields' in keys:
            ...
            # for darks, don't want to divide out inter_bright for counting projections
            ndark = dxchange.reader._count_proj(dgroup, dgroup.name.split('/')[-1] + 'drk_0000.tif',
                                                numangles,
                                                inter_bright=-1)
        ...
        if 'num_bright_field' in keys:
            ...
            nflat = dxchange.reader._count_proj(dgroup, dgroup.name.split('/')[-1] + 'bak_0000.tif',
                                                numangles,
        ...
        if anglelist is None:
            # the offset angle should offset from the angle of the first image, which is usually 0,
            # but in the case of timbir data may not be.
        ...
        numangles = int(dxchange.read_hdf5(os.path.join(inputPath, filename),
                                           "/process/acquisition/rotation/num_angles")[0])
        angularrange = dxchange.read_hdf5(os.path.join(inputPath, filename),
                                          "/process/acquisition/rotation/range")[0]
        anglelist = dxchange.read_hdf5(os.path.join(inputPath, filename),
                                       '/exchange/theta', slc=None)
        if anglelist is None:
            try:
                ...
                print(f"An error occurred while reading anglelist: {e}")
        anglelist = anglelist - angle_offset
        anglelist = np.deg2rad(anglelist)
        anglelist = -anglelist
        numslices = int(dxchange.read_hdf5(os.path.join(inputPath, filename),
                                           "/measurement/instrument/detector/dimension_y")[0])
        numrays = int(dxchange.read_hdf5(os.path.join(inputPath, filename),
                                         "/measurement/instrument/detector/dimension_x")[0])
        pxsize = dxchange.read_hdf5(os.path.join(inputPath, filename),
                                    # /10 to convert units from mm to cm
                                    "/measurement/instrument/detector/pixel_size")[0] / 10.0
        inter_bright = int(dxchange.read_hdf5(os.path.join(inputPath, filename),
                                              "/process/acquisition/flat_fields/i0cycle")[0])
        group_flat = [0, numangles - 1]
        nflat = int(dxchange.read_hdf5(os.path.join(inputPath, filename),
                                       "/process/acquisition/flat_fields/num_flat_fields")[0])
        ind_flat = list(range(0, nflat))
        ndark = int(dxchange.read_hdf5(os.path.join(inputPath, filename),
                                       "/process/acquisition/dark_fields/num_dark_fields")[0])
        ind_dark = list(range(0, ndark))
        propagation_dist = dxchange.read_hdf5(
            os.path.join(inputPath, filename),
            "/measurement/instrument/camera_motor_stack/setup/camera_distance")[0]
        if (propagation_dist == 0):
            propagation_dist = dxchange.read_hdf5(
                os.path.join(inputPath, filename),
                "/measurement/instrument/camera_motor_stack/setup/camera_distance")[1]
        kev = dxchange.read_hdf5(os.path.join(inputPath, filename),
                                 "/measurement/instrument/monochromator/energy")[0] / 1000
        if (kev == 0):
            kev = dxchange.read_hdf5(os.path.join(inputPath, filename),
        ...
        _, _, _, anglelist = read_sls(os.path.join(inputPath, filename),
                                      exchange_rank=0,
                                      proj=(timepoint*numangles, (timepoint+1)*numangles, 1),
                                      sino=(0, 1, 1))  # dtype=None, , )
    ...
    if projused is not None and (projused[1] > numangles - 1 or projused[0] < 0):
        if projused[1] > numangles:
            message = ("End Projection value greater than number of angles. "
                       "Value has been lowered to the number of angles ")
            print(message + str(numangles))
    ...
    if projused is None:
        ...
        # if projused is different than default, need to change numangles and angularrange;
    ...
    # figure out how user can pass to do central x number of slices, or set of slices dispersed throughout
    # (without knowing a priori the value of numslices)
    if sinoused is None:
    ...
    if verbose_printing:
        print('There are ' + str(numslices) + ' sinograms, ' + str(numrays) +
              ' rays, and ' + str(numangles) + ' projections, with an angular range of ' +
              str(angularrange) + '.')

        print('Looking at sinograms ' + str(sinoused[0]) + ' through ' +
              str(sinoused[1]-1) + ' (inclusive) in steps of ' + str(sinoused[2]))

    BeamHardeningCoefficients = (
                                    (0, 1, 0, 0, 0, .004)
                                    if BeamHardeningCoefficients is None
                                    else BeamHardeningCoefficients)
    ...
    if cor is None:
        ...
        with warnings.catch_warnings():
            ...
            if (filetype == 'als') or (filetype == 'als1131'):
                if corLoadMinimalBakDrk:
                    ...
                    ind_flat = 0
                if filetype == 'als':
                    tomo, flat, dark, floc = dxchange.read_als_832h5(os.path.join(inputPath, filename),
                                                                     ind_tomo=(0, lastcor),
                                                                     ind_dark=ind_dark,
                                                                     ind_flat=ind_flat)
                else:
                ...
                #     ind_dark = 0
                #     ind_flat = 0
                # tomo, flat, dark, coranglelist, _ = dxchange.exchange.read_dx(os.path.join(inputPath, filename),
                # proj=(0,numangles-1),ind_dark=ind_dark,ind_flat=ind_flat)
                # tomo, flat, dark, coranglelist, _ = dxchange.exchange.read_dx(os.path.join(inputPath, filename),
                # proj=(0,lastcor,lastcor-1))
                tomo, flat, dark, coranglelist = dxchange.exchange.read_aps_tomoscan_hdf5(os.path.join(inputPath,
                                                                                                       filename),
                                                                                          proj=(0,
                                                                                                lastcor,
                                                                                                lastcor-1))
            elif (filetype == 'sls'):
                tomo, flat, dark, coranglelist = read_sls(os.path.join(inputPath, filename),
                                                          exchange_rank=0,
                                                          proj=(timepoint * numangles,
                                                                (timepoint + 1) * numangles,
                                                                numangles - 1))  # dtype=None, , )
            else:
                return
        ...
        if corFunction == 'vo':
            ...
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                if (filetype == 'als'):
                    tomovo, flat, dark, floc = dxchange.read_als_832h5(os.path.join(inputPath, filename),
                                                                       sino=(sinoused[0],
                                                                             sinoused[0]+1, 1))
                elif (filetype == 'sls'):
                    tomovo, flat, dark, coranglelist = read_sls(os.path.join(inputPath, filename), exchange_rank=0,
                                                                sino=(sinoused[0], sinoused[0]+1, 1),
                                                                proj=(timepoint*numangles+projused[0],
                                                                      timepoint*numangles+projused[1],
                                                                      projused[2]))  # dtype=None, , )
                else:
                    return
                if bffilename is not None:
                    if (filetype == 'als'):
                        tomobf, flatbf, darkbf, flocbf = dxchange.read_als_832h5(os.path.join(inputPath,
                                                                                              bffilename),
                                                                                 sino=(sinoused[0],
                                                                                       sinoused[0]+1, 1))
                        flat = tomobf
            ...
            with warnings.catch_warnings():
                ...
                if (filetype == 'als'):
                    ...
                    tomo, flat, dark, coranglelist = dxchange.read_aps_tomoscan_hdf5(os.path.join(inputPath,
                                                                                                  filename),
                                                                                     exchange_rank=0, proj=(
                        0, lastcor, lastcor-1))  # dtype=None, , )
                elif (filetype == 'sls'):
                    tomo, flat, dark, coranglelist = read_sls(os.path.join(inputPath, filename), exchange_rank=0,
                                                              proj=(
    ...
    recon_dict = {
        ...
        "butterworth_cutoff": butterworth_cutoff,  # 0.1 would be very smooth,
                                                   # 0.4 would be very grainy (reconstruction)
        ...
        "projused": projused,  # should be slicing in projection dimension (start,end,step)
                               # Be sure to add one to the end as stop in python means the last value is omitted
        "sinoused": sinoused,  # should be sliceing in sinogram dimension (start,end,step).
                               # If first value is negative, it takes the number of slices from the second value
        ...
        "angle_offset": angle_offset,  # this is the angle offset from our default (270) so that tomopy yields
                                       # output in the same orientation as previous software (Octopus)
        "anglelist": anglelist,  # if not set, will assume evenly spaced angles which will be calculated by the
                                 # angular range and number of angles found in the file. if set to -1, will read
                                 # individual angles from each image. alternatively, a list of angles can be passed
        "doBeamHardening": doBeamHardening,  # turn on beam hardening correction, based on "Correction for beam
                                             # hardening in computed tomography"
                                             # Gabor Herman, 1979 Phys. Med. Biol. 24 81
        "BeamHardeningCoefficients": BeamHardeningCoefficients,  # 6 values,
                                                                 # tomo = a0 + a1*tomo + a2*tomo^2
                                                                 # + a3 * tomo^3 + a4*tomo^4 + a5*tomo^5
        "projIgnoreList": projIgnoreList,  # projections to be ignored in the reconstruction (for simplicity in
                                           # the code, they will not be removed and will be processed as all other
                                           # projections but will be set to zero absorption right before recon.
...
def recon(
    ...
    bffilename=None,
    inputPath='./',  # input path, location of the data set to reconstruct
    outputFilename=None,  # file name for output tif files (a number and .tiff will be added).
    ...
    chunk_sino=100,  # chunk size in sinogram direction
    npad=None,  # amount to pad data before reconstruction
    projused=None,  # should be slicing in projection dimension (start,end,step) Be sure to add one to the end as
                    # stop in python means the last value is omitted
    sinoused=None,  # should be sliceing in sinogram dimension (start,end,step). If first value is negative,
                    # it takes the number of slices from the second value in the middle of the stack.
    correcttilt=0,  # tilt dataset
    tiltcenter_slice=None,  # tilt center (x direction)
    tiltcenter_det=None,  # tilt center (y direction)
    angle_offset=0,  # this is the angle offset from our default (270) so that tomopy yields output in the same
                     # orientation as previous software (Octopus)
    anglelist=None,  # if not set, will assume evenly spaced angles which will be calculated by the angular range
                     # and number of angles found in the file. if set to -1, will read individual angles from each
                     # image. alternatively, a list of angles can be passed.
    doBeamHardening=False,  # turn on beam hardening correction, based on "Correction for beam hardening in
                            # computed tomography", Gabor Herman, 1979 Phys. Med. Biol. 24 81
    BeamHardeningCoefficients=(0, 1, 0, 0, 0, .1),  # 6 values,
                                                    # tomo = a0 + a1*tomo + a2*tomo^2
                                                    # + a3*tomo^3 + a4*tomo^4 + a5*tomo^5
    projIgnoreList=None,  # projections to be ignored in the reconstruction (for simplicity in the code, they will
                          # not be removed and will be processed as all other projections but will be set to zero
    ...
    *args, **kwargs
    ...
    for tmpfile in tempfilenames:
        try:
            ...
            pass
    # number of total projections. We add 1 to include the last projection
    numprojused = len(range(projused[0], projused[1], projused[2]))
    # number of total sinograms. We add 1 to include the last projection
    numsinoused = len(range(sinoused[0], sinoused[1], sinoused[2]))
    # sets the chunk size to either all of the projections used or the chunk size
    num_proj_per_chunk = np.minimum(chunk_proj, numprojused)
    # adding 1 fixes the case of the number of projections not being a factor of the chunk size.
    # Subtracting 1 fixes the edge case where the number of projections is a multiple of the chunk size
    ...
    # adding 1 fixes the case of the number of sinograms not being a factor of the chunk size.
    # Subtracting 1 fixes the edge case where the number of sinograms is a multiple of the chunk size
    numsinochunks = (numsinoused - 1) // num_sino_per_chunk + 1
    ...
    while True:  # Loop over reading data in certain chunking direction
        ...
        for y in range(niter):  # Loop over chunks
            ...
            # The standard case. Unless the combinations below are in our function list, we read darks and flats
            # normally, and on next chunck proceed to "else."
            if curfunc == 0 and not (
                                    ('normalize_nf' in function_list and 'remove_outlier2d' in function_list) or
                                    ('remove_outlier1d' in function_list and 'remove_outlier2d' in function_list)):
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    if axis == 'proj':
                        if (filetype == 'als'):
                            min = np.minimum((y + 1)*projused[2]*num_proj_per_chunk+projused[0],
                                             projused[1])
                            tomo, flat, dark, floc = dxchange.read_als_832h5(os.path.join(inputPath, filename),
                                                                             ind_tomo=range(y*projused[2]
                                                                                            * num_proj_per_chunk
                                                                                            + projused[0], min,
                                                                                            projused[2]),
                                                                             sino=(sinoused[0],
                            ...
                            del min
                            if bffilename is not None:
                                tomobf, _, _, _ = dxchange.read_als_832h5(os.path.join(inputPath, bffilename),
                                                                          sino=(sinoused[0],
                                                                                sinoused[1],
                                                                                sinoused[2]))
                                # I don't think we need this for separate bf:
                                # ind_tomo=range(y*projused[2]*num_proj_per_chunk+projused[0],
                                # np.minimum((y + 1)*projused[2]*num_proj_per_chunk+projused[0],
                                # projused[1]),projused[2]),
                                flat = tomobf
                        elif (filetype == 'als1131'):
                            min = np.minimum((y + 1)*projused[2]*num_proj_per_chunk+projused[0], projused[1])
                            tomo, flat, dark, floc = read_als_1131h5(os.path.join(inputPath, filename),
                                                                     ind_tomo=range((y*projused[2]
                                                                                    * num_proj_per_chunk
                                                                                    + projused[0]),
                            ...
                            del min
                            if bffilename is not None:
                                tomobf, _, _, _ = read_als_1131h5(os.path.join(inputPath, bffilename),
                                                                  sino=(sinoused[0], sinoused[1], sinoused[2]))
                                # I don't think we need this for separate bf:
                                # ind_tomo=range(y*projused[2]*num_proj_per_chunk+projused[0],
                                # np.minimum((y + 1)*projused[2]*num_proj_per_chunk+projused[0],projused[1]),
                                # projused[2]),
                                flat = tomobf
                        elif (filetype == 'dxfile'):
                            proj = (y * projused[2] * num_proj_per_chunk + projused[0],
                                    np.minimum((y+1)*projused[2] * num_proj_per_chunk + projused[0], projused[1]),
                                    projused[2])
                            tomo, flat, dark, _ = dxchange.exchange.read_aps_tomoscan_hdf5(os.path.join(inputPath,
                                                                                                        filename),
                                                                                           exchange_rank=0,
                                                                                           proj=proj,
                                                                                           sino=sinoused)
                            ...
                            tomo, flat, dark, _ = read_sls(os.path.join(inputPath, filename),
                                                           exchange_rank=0,
                                                           proj=(timepoint*numangles+y*projused[2]
                                                                 * num_proj_per_chunk+projused[0],
                                                                 timepoint*numangles
                                                                 + np.minimum((y + 1)*projused[2]
                                                                              * num_proj_per_chunk+projused[0],
                                                                              projused[1]),
                                                                 projused[2]),
                        ...
                        if (filetype == 'als'):
                            ...
                            if bffilename is not None:
                                tomobf, _, _, _ = dxchange.read_als_832h5(os.path.join(inputPath, bffilename),
                                                                          sino=(y*sinoused[2]*num_sino_per_chunk
                                                                                + sinoused[0],
                                                                                np.minimum((y + 1)*sinoused[2]
                                                                                           * num_sino_per_chunk
                                                                                           + sinoused[0],
                                                                                           sinoused[1]),
                                                                                sinoused[2]))
                            ...
                            tomo, flat, dark, _ = dxchange.exchange.read_aps_tomoscan_hdf5(
                                os.path.join(inputPath, filename),
                                exchange_rank=0,
                                proj=(projused[0], projused[1], projused[2]),
                                sino=(y * sinoused[2] * num_sino_per_chunk + sinoused[0],
                                      np.minimum(
                                        (y + 1) * sinoused[2] * num_sino_per_chunk +
                                        sinoused[0], sinoused[1]), sinoused[2]))  # dtype=None, , )
                        elif (filetype == 'sls'):
                            tomo, flat, dark, _ = read_sls(os.path.join(inputPath, filename),
                                                           exchange_rank=0,
                                                           proj=(timepoint*numangles+projused[0], timepoint
                                                                 * numangles+projused[1], projused[2]),
                                                           sino=(y*sinoused[2]*num_sino_per_chunk+sinoused[0],
                                                                 np.minimum((y + 1)*sinoused[2]*num_sino_per_chunk
                                                                            + sinoused[0], sinoused[1]),
                                                                 sinoused[2]))  # dtype=None, , )
                        else:
                            break
            # Handles the initial reading of scans. Flats and darks are not read in,
            # because the chunking direction will swap before we normalize. We read in darks when we normalize.
            elif curfunc == 0:
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore")
                    if axis == 'proj':
                        if (filetype == 'als') or (filetype == 'als1131'):
                            tomo = read_als_h5_tomo_only(os.path.join(inputPath, filename),
                                                         ind_tomo=range((y*projused[2]
                                                                         * num_proj_per_chunk+projused[0]),
                                                                        np.minimum(
                                                                            (y + 1)*projused[2]*num_proj_per_chunk
                                                                            + projused[0], projused[1]),
                                                                        projused[2]),
                                                         sino=(sinoused[0], sinoused[1], sinoused[2]), bl=filetype)
                        elif (filetype == 'dxfile'):
                            tomo, _, _, _, _ = read_sls(os.path.join(inputPath, filename),
                                                        exchange_rank=0,
                                                        proj=(y * projused[2] * num_proj_per_chunk + projused[0],
                                                              np.minimum((y + 1) * projused[2]
                                                                         * num_proj_per_chunk + projused[0],
                            ...
                            tomo, _, _, _ = read_sls(os.path.join(inputPath, filename),
                                                     exchange_rank=0,
                                                     proj=(timepoint*numangles+y*projused[2]*num_proj_per_chunk
                                                           + projused[0],
                                                           timepoint*numangles
                                                           + np.minimum(
                                                               (y + 1)*projused[2]*num_proj_per_chunk+projused[0],
                                                               projused[1]),
                                                           projused[2]),
                                                     sino=sinoused)  # dtype=None, , )
                        else:
                        ...
                        if (filetype == 'als') or (filetype == 'als1131'):
                            tomo = read_als_h5_tomo_only(os.path.join(inputPath, filename),
                                                         ...
                                                         sino=(y*sinoused[2]*num_sino_per_chunk+sinoused[0],
                                                               np.minimum(
                                                                   ((y + 1)*sinoused[2]*num_sino_per_chunk
                                                                    + sinoused[0]), sinoused[1]), sinoused[2]),
                                                         bl=filetype)
                        elif (filetype == 'dxfile'):
                            tomo, _, _, _ = dxchange.exchange.read_aps_tomoscan_hdf5(os.path.join(inputPath,
                                                                                                  filename),
                                                                                     exchange_rank=0, proj=(
                                                                                         projused[0],
                                                                                         projused[1],
                            ...
                            tomo, _, _, _ = read_sls(os.path.join(inputPath, filename),
                                                     ...
                                                     sino=(y*sinoused[2]*num_sino_per_chunk+sinoused[0],
                                                           np.minimum(((y + 1)*sinoused[2]
                                                                       * num_sino_per_chunk+sinoused[0]),
                                                           sinoused[1]),
                                                           sinoused[2]))  # dtype=None, , )
                        else:
                            break
            # Handles the reading of darks and flats,
            # once we know the chunking direction will not change before normalizing.
            elif ('remove_outlier2d' == function_list[curfunc] and
                  'normalize' in function_list) or ('normalize_nf' == function_list[curfunc]):
                if axis == 'proj':
                    ...
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        if (filetype == 'als') or (filetype == 'als1131'):
                            flat, dark, floc = read_als_h5_non_tomo(os.path.join(inputPath, filename),
                                                                    ind_tomo=range((y*projused[2]
                                                                                    * num_proj_per_chunk
                                                                                    + projused[0]),
                                                                                   np.minimum(
                                ...
                                if filetype == 'als':
                                    tomobf, _, _, _ = dxchange.read_als_832h5(os.path.join(inputPath, bffilename),
                                                                              sino=(sinoused[0],
                                                                                    sinoused[1],
                                                                                    sinoused[2]))
                                    # I don't think we need this since it is full tomo in separate file:
                                    # ind_tomo=range(y*projused[2]*num_proj_per_chunk+projused[0],
                                    # np.minimum((y + 1)*projused[2]*num_proj_per_chunk+projused[0],projused[1]),
                                    # projused[2])
                                    flat = tomobf
                            ...
                            min = np.minimum((y + 1) * projused[2] * num_proj_per_chunk + projused[0], projused[1])
                            _, flat, dark, _ = dxchange.exchange.read_aps_tomoscan_hdf5(os.path.join(inputPath,
                                                                                                     filename),
                                                                                        exchange_rank=0,
                                                                                        proj=(y * projused[2]
                                                                                              * num_proj_per_chunk
                            ...
                            _, flat, dark, _ = read_sls(os.path.join(inputPath, filename),
                                                        exchange_rank=0,
                                                        proj=(timepoint*numangles+y*projused[2]*num_proj_per_chunk
                                                              + projused[0],
                                                              timepoint*numangles
                    ...
                    tomo = dxchange.reader.read_hdf5(tempfilenames[curtemp],
                                                     '/tmp/tmp',
                                                     slc=((0, numangles, 1), (start, end, 1), (0, numrays, 1)))
                    with warnings.catch_warnings():
                        warnings.simplefilter("ignore")
                        if (filetype == 'als') or (filetype == 'als1131'):
                            flat, dark, floc = read_als_h5_non_tomo(os.path.join(inputPath, filename),
                                                                    ...
                                                                    sino=(y*sinoused[2]*num_sino_per_chunk
                                                                          ...
                                                                          np.minimum((y + 1)*sinoused[2]
                                                                                     * num_sino_per_chunk
                                                                                     + sinoused[0],
                                                                                     sinoused[1]), sinoused[2]),
                                                                    bl=filetype)
                        elif (filetype == 'dxfile'):
                            _, flat, dark, _, _ = read_sls(os.path.join(inputPath, filename),
                                                           exchange_rank=0,
                                                           proj=(projused[0],  projused[1], projused[2]),
                                                           sino=(y*sinoused[2]*num_sino_per_chunk+sinoused[0],
                                                                 np.minimum((y + 1) * sinoused[2]
                            ...
                            break
            # Anything after darks and flats have been read or the case in which remove_outlier2d is the current
            # /2nd function and the previous case fails.
            else:
                if axis == 'proj':
                    start, end = y * num_proj_per_chunk, np.minimum((y + 1) * num_proj_per_chunk, numprojused)
                    # read in intermediate file
                    tomo = dxchange.reader.read_hdf5(tempfilenames[curtemp], '/tmp/tmp',
                                                     slc=((start, end, 1), (0, numslices, 1), (0, numrays, 1)))
                else:
                    start, end = y * num_sino_per_chunk, np.minimum((y + 1) * num_sino_per_chunk, numsinoused)
                    tomo = dxchange.reader.read_hdf5(tempfilenames[curtemp], '/tmp/tmp',
                                                     slc=((0, numangles, 1), (start, end, 1), (0, numrays, 1)))
            ...
            while True:  # Loop over operations to do in current chunking direction
                ...
                if newaxis != 'both' and newaxis != axis:
                    ...
                    dxchange.writer.write_hdf5(tomo, fname=tempfilenames[1-curtemp], gname='tmp', dname='tmp',
                ...
                if func_name == 'write_raw':
                    dxchange.write_tiff_stack(tomo, fname=filenametowrite,
                                              start=y * num_proj_per_chunk + projused[0])
                    if y == 0:
                    ...
                    tomo = tomo.astype(np.float32, copy=False)
                    # use floc_independent b/c when you read file in proj chunks, you don't get the correct floc
                    # returned right now to use here.
                    tomopy.normalize_nf(tomo, flat, dark, floc_independent, out=tomo)
                    if bfexposureratio != 1:
                        if verbose_printing:
                    ...
                    # setting min %transmission to 1% helps avoid streaking from very high absorbing areas
                    ...
                    loc_dict = {'a{}'.format(i): np.float32(val) for i,
                                val in enumerate(BeamHardeningCoefficients)}
                    loc_dict['tomo'] = tomo
                    tomo = ne.evaluate('a0 + a1*tomo + a2*tomo**2 + a3*tomo**3 + a4*tomo**4 + a5*tomo**5',
                                       local_dict=loc_dict, out=tomo)
                elif func_name == 'remove_stripe_fw':
                    tomo = tomopy.remove_stripe_fw(tomo, sigma=ringSigma, level=ringLevel, pad=True,
                                                   wname=ringWavelet)
                    ...
                    for b in range(tomo.shape[0]):
                        tomo[b] = st.rotate(tomo[b], correcttilt, center=cntr, preserve_range=True,
                                            order=1, mode='edge', clip=True)
                        # center=None means image is rotated around its center; order=1 is default,
                    ...
                    tomo = tomopy.prep.alignment.distortion_correction_proj(tomo, lensdistortioncenter[0],
                                                                            lensdistortioncenter[1],
                                                                            lensdistortionfactors,
                                                                            ncore=None, nchunk=None)
                elif func_name == 'do_360_to_180':

                    # Keep values around for processing the next chunk in the list
                    keepvalues = [angularrange, numangles, projused, num_proj_per_chunk, numprojchunks,
                                  numprojused, numrays, anglelist]

                    # why -.5 on one and not on the other?
                    if tomo.shape[0] % 2 > 0:
                        tomo = sino_360_to_180(tomo[0:-1, :, :],
                                               overlap=int(np.round((tomo.shape[2]-cor-.5))*2),
                                               rotation='right')
                        angularrange = angularrange/2 - angularrange/(tomo.shape[0]-1)
                    else:
                        tomo = sino_360_to_180(tomo[:, :, :],
                    ...
                    if projIgnoreList is not None:
                        for badproj in projIgnoreList:
                            tomo[badproj] = 0
                    # rec = tomopy.recon(tomo, anglelist, center=cor+npad, algorithm=recon_algorithm,
                    # filter_name='butterworth', filter_par=[butterworth_cutoff, butterworth_order])
                    rec = tomopy.recon(tomo, anglelist, accelerated=True, device='gpu', center=cor+npad,
                                       algorithm=recon_algorithm, filter_name='butterworth',
                    ...
                    rec = tomopy.remove_ring(rec, theta_min=Rarc, rwidth=Rmaxwidth, thresh_max=Rtmax, thresh=Rthr,
                                             thresh_min=Rtmin, out=rec)
                elif func_name == 'polar_ring2':
                    rec = np.ascontiguousarray(rec, dtype=np.float32)
                    rec = tomopy.remove_ring(rec, theta_min=Rarc2, rwidth=Rmaxwidth2, thresh_max=Rtmax2,
                                             thresh=Rthr2, thresh_min=Rtmin2, out=rec)
                elif func_name == 'castTo8bit':
                    rec = convert8bit(rec, cast8bit_min, cast8bit_max)
                elif func_name == 'write_reconstruction':
                    if dorecon:
                        if sinoused[2] == 1:
                            dxchange.write_tiff_stack(rec, fname=filenametowrite,
                                                      start=y*num_sino_per_chunk + sinoused[0])
                        else:
                            ...
                            for sinotowrite in rec:    # fixes issue where dxchange only writes for step sizes of 1
                                dxchange.writer.write_tiff(sinotowrite,
                                                           fname=filenametowrite + '_' + '{0:0={1}d}'.format(num,
                                                                                                             5))
                                # Set permissions for individual files
                                set_file_permissions(filenametowrite + '_' + '{0:0={1}d}_norm'.format(num, 5))
                                num += sinoused[2]
                    else:
                        if verbose_printing:
                    ...
                    if projused[2] == 1:
                        dxchange.write_tiff_stack(tomo, fname=filenametowrite+'_norm',
                                                  start=y * num_proj_per_chunk + projused[0])
                    else:
                        ...
                        for projtowrite in tomo:  # fixes issue where dxchange only writes for step sizes of 1
                            dxchange.writer.write_tiff(projtowrite,
                                                       fname=filenametowrite + '_' + '{0:0={1}d}_norm'.format(num,
                                                                                                              5))
                            # Set permissions for individual files
                            set_file_permissions(filenametowrite + '_' + '{0:0={1}d}_norm'.format(num, 5))
                            num += projused[2]
            ...
            if y < niter-1 and keepvalues:  # Reset original values for next chunk
                ...
                numprojchunks, numprojused, numrays, anglelist = keepvalues
...
def translate(data, dx=0, dy=0, interpolation=True):
    """
    Shifts all projections in an image stack by dx (horizontal) and dy (vertical) pixels.
    Translation with subpixel resolution is possible with interpolation==True
    ...
    if interpolation is False:
        ...
        for n in range(Nproj):
            ...
            # shift image by dx and dy, replace original without padding
            dataOut[n, :, :] = PaddedImage[Npad-drow:Npad+Nrow-drow, Npad-dcol:Npad+Ncol-dcol]
...
def linear_translation_correction(data, dx=100.5, dy=700.1, interpolation=True):
    ...
    """
    Corrects for a linear drift in field of view (horizontal dx, vertical dy) over time.
    The first index indicates time data[time,:,:] in the time series of projections.
    dx and dy are the final shifts in FOV position.
    ...
    if interpolation is True:
        for n in range(Nproj):
            # Generate Translation Function based on dy[n] and dx[n]
            # translateFunction = st.SimilarityTransform(translation=(-dx_n[n],dy_n[n]))
            M = np.matrix([[1, 0, -dx_n[n]], [0, 1, dy_n[n]], [0, 0, 1]])
            translateFunction = st.SimilarityTransform(matrix=M)
            image_n = data[n, :, :]
            # Apply translation with interpolation to projection[n]
            dataOut[n, :, :] = st.warp(image_n, translateFunction)
            # print(n)

    if interpolation is False:
        Npad = max(dx, dy)
        for n in range(Nproj):
            # copy single projection and pad with maximum of dx,dy
            PaddedImage = np.pad(data[n, :, :], Npad, 'constant')
...
This processes the (roughly) central 200 slices, and saves it to a new file. The vertical speed is one of the input
arguments, and I simply estimate it manually by looking at the first and last projection,
shifting them by 'np.roll'. The input argument is the total number of pixels that are shifted over the whole scan
(which is then converted to pixels per projection by dividing by the number of projections-1).
I don't really remember why I wrote my own code, but maybe I was running into problems using scikit-image as well.
The current code uses linear interpolation, and gives pretty good results for the data I tested.
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python reconstruction.py <file_name> <folder_path>")
        sys.exit(1)
    ...
    recon_dictionary, _ = recon_setup(filename=file_name,
                                      ...
                                      outputPath="../scratch/"+folder_path)  # sinoused = (-1,1,1))
